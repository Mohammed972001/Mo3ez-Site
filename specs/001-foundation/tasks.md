...
| T12 | نظام المدوّنة — الأساس بلا مقالات | ✅ |
...
**التنفيذ (تصميم ذاتي — لا ريفرنس):** `lib/blog/posts.ts` نموذج بيانات (`Post` = بيانات وصفية + أقسام عنوان/فقرات) + مصفوفة **فارغة عمدًا** + مساعدات (`allPosts`/`getPost`/`postPath`). `app/blog/page.tsx` فهرس بشبكة بطاقات (`.blog-grid`) و**حالة فراغ مصمَّمة** («مقالات قيّمة في الطريق» + CTA واتساب) عند عدم وجود مقالات. `app/blog/[slug]/page.tsx` قالب مقال (SSG عبر `generateStaticParams` — فارغ الآن) + غلاف + أقسام بـ `.prose` + `generateMetadata` + **Article + BreadcrumbList JSON-LD**. لإضافة مقال: عنصر واحد في `posts`. **تحقّق Playwright:** حالة الفراغ تظهر، بلا overflow، الواتساب FAB بالشعار الصحيح. `next build` ✅ · `tsc` نظيف. (قابل للترقية لـ MDX لاحقًا لو لزمت عناصر غنية.)

> بهذا لم يبقَ أي رابط 404 في النافبار/الفوتر. المتبقّي: T11 (فورم عرض سعر) · T09 (صفحات الفئة) · T14–T17.
